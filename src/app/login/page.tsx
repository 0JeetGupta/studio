  ...
  const handleFaceVerification = () => {
    ...
    if (!storedFaceData) {
      toast({
        ...
        description: 'Please register your face in the Settings page first.',
      });
      return;
    }
  ...
  const handleScanComplete = (result: FaceScanResult) => {
    ...
    if (result.status === 'success' && result.descriptor) {
      ...
      if (storedFaceData) {
        ...
        if (bestMatch.label === 'match') {
          toast({
            title: 'Face Verified!',
            description: 'You can now log in. This feature is a demo and does not log you in automatically.',
          });
        } else {
          toast({
            variant: 'destructive',
            title: 'Verification Failed',
            description: 'Face does not match the registered user. Please try again.',
          });
        }
      }
    } else {
      toast({
        variant: 'destructive',
        title: 'Scan Failed',
        description: result.message || 'Could not verify face. Please try again.',
      });
    }
  };
